...
toolbar.onAdd = function () {
  ...
  // Area button
  var areaBtn = L.DomUtil.create("button", "map-btn", div);
  areaBtn.type = "button";
  areaBtn.innerHTML = "Area";
  ...
  // Button handlers
  L.DomEvent.on(zoomInBtn, "click", function () {
    map.zoomIn();
  });
  L.DomEvent.on(zoomOutBtn, "click", function () {
    map.zoomOut();
  });
  ...
  L.DomEvent.on(measureBtn, "click", function () {
    ...
    if (measureCtrl && measureCtrl.start) {
      measureCtrl.start();
    }
    if (areaCtrl && areaCtrl.stop) {
      areaCtrl.stop();
  ...
  L.DomEvent.on(areaBtn, "click", function () {
    areaBtn.classList.add("active");
    measureBtn.classList.remove("active");
    panBtn.classList.remove("active");
    map.dragging.disable();
    if (areaCtrl && areaCtrl.start) {
      areaCtrl.start();
    }
    if (measureCtrl && measureCtrl.stop) {
      measureCtrl.stop();
    }
    toolbar._rulerActive = true;
  });
  L.DomEvent.on(clearBtn, "click", function () {
    try {
      if (measureCtrl && measureCtrl.clear) {
        measureCtrl.clear();
      }
      if (areaCtrl && areaCtrl.clear) {
        areaCtrl.clear();
      }
    } catch (e) {
      /* no-op */
  ...
  L.DomEvent.on(printBtn, "click", function () {
    try {
      if (
        !measureCtrl ||
        !measureCtrl.latlngsList ||
        !measureCtrl.latlngsList.length
      )
        return;
      ...
      var doc = [
        ...
        '<div class="muted">Total Length: ' +
          ...
          "</div>",
        '<div id="map"></div>',
        '<div class="card"><strong>Segments</strong><ul>' +
          lines.join("") +
          "</ul></div>",
  ...
  L.DomEvent.on(finishBtn, "click", function () {
    try {
      if (measureCtrl && measureCtrl.finish) {
        measureCtrl.finish();
      }
    } catch (e) {}
  });
...
// Polygon area control
var areaCtrl = new L.Control.AreaMeasurement({
  color: "#d17b0f",
  fillColor: "#d17b0f",
  defaultUnit: "m2",
});
map.addControl(areaCtrl);
// Hide the default control button; we use our own toolbar
if (measureCtrl && measureCtrl._container) {
  try {
    measureCtrl._container.style.display = "none";
  } catch (e) {}
}
if (areaCtrl && areaCtrl._container) {
  try {
    areaCtrl._container.style.display = "none";
  } catch (e) {}
}
